...
CONFIG_FILE = 'smartlingzd.ini'
TRANSFER_CONFIG_FILE = 'translate.ini'
...
def construct_section_translation(translation_data, zd_locale):
    ...
    translation['locale'] = zd_locale
    translation['title'] = translation_data['name'] # title seems to be required
...
def transfer_all_translations_from_smartling(item_type, zd_locales, 
    ...
    # Then for each of those, if it appears on the list of items for which we need 
    # translations, transfer it over.

    for zd_locale in zd_locales:
        sl_locale = get_smartling_locale(zd_locale)

        # TODO call get_completed_ids once including all locales
...
def remove_nbsps(item, item_type):
    """ Remove NBSPs from translatable fields because they're being inadvertently
    created and are impacting smart match. Keep 1 if there are two together
    in order to provide a way to have them if you want them.
    """

    for field in get_fields_to_translate(item_type):
        if item[field] is not None:
            item[field] = item[field].replace(u'&nbsp;&nbsp;', u'&nbsp;REPLACEWITHNBSP')
            item[field] = item[field].replace(u'&nbsp;', u' ')
            item[field] = item[field].replace(u'REPLACEWITHNBSP', u'&nbsp;')
